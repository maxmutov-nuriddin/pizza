    ...
    <section className="sell my-5">
      <div className="sell__order">
        ...
        <div className=" d-flex flex-column gap-4 mt-5">
          {
            values.length === 0 ? <h4>Заказов нет</h4> : values.map((value, i) => (
              <div key={i} className="sell__item d-flex flex-wrap flex-row align-items-center justify-content-between">
                ...
                <div className="sell__item-controls d-flex align-items-center gap-2">
                  <button onClick={() => handleCardClicks(value.id)} className="sell__item-button sell__item-button--plus">+</button>
                  <p>{value.quantity}</p>
                  <button onClick={() => handleCardClickMinus(value.id)} className="sell__item-button sell__item-button--minus">-</button>
                </div>
                <strong className="sell__item-price">{value.quantity * value.price} ₽</strong>
              </div>
            ))
          }
        </div>
